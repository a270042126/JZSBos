...
<script type="text/javascript">
			...
			toolbar : [
				{
					...
					iconCls : 'icon-add',
					handler : function(){
						location.href = "${pageContext.request.contextPath}/workflow/processdefinition_deploy.action";
					}
				}
			]
		});
	...
	function del(id){
		$.messager.confirm("确认信息","你确定删除当前流程定义吗？",function(r){
			if(r){
				//发送请求，删除数据
				window.location.href = '${pageContext.request.contextPath }/processDefinition/delete.action?id=' + id;
			}
		});
	}
</script>
  		...
  		<tbody>
  			<s:iterator value="list" var="processDefinition">
  				...
  				<tr>
  					...
  					<td>
  						<a onclick="window.showModalDialog('${pageContext.request.contextPath }/processDefinition/showpng.action?id=${id }')"
